...
class AddTrackingItemPresenter(
    ...
    override fun fetchRecommendShippingCompany() {
        scope.launch {
            ...
            shippingCompanyRepository.getRecommendShippingCompany(invoice!!)
                ?.let { view.showRecommendCompany(it) }
    ...
    override fun saveTrackingItem() {
        scope.launch {
            try {
                ...
                view.showErrorToast(exception.message ?: "서비스에 문제가 생겨서 운송장을 추가하지 못햇어요")
            } finally {
